<?php

global $post;


?>

<!-- Main Slider -->

<div id="sequence">
	<span class="sequence-prev glyphicon glyphicon-chevron-left" alt="Previous"></span>
	<span class="sequence-next glyphicon glyphicon-chevron-right" alt="Next"></span>

	 <ul class="sequence-canvas">

         <?php for($i = 1 ; $i < 5 ; $i++) { ?>
		 <li class="animate-in slider-set-<?php echo $i ;?>">
		 	<div class="info">
		 		<h2><?php echo get_meta_data_singuler( 'nineteen_slider_'.$i );?></h2>
		 		<p><?php echo get_meta_data_singuler( 'nineteen_slider_'.$i );?></p>
		  	</div>
		 	<img class="sky" src="<?php print IMAGES; ?>/banners/banner-01.jpg" alt="Blue Sky" /> <img class="balloon" src="<?php print IMAGES; ?>/banners/banner-slider-parts-01.png" alt="Balloon" /> 
		 </li>

		 <?php } ?>

	 </ul>
</div>
<div class="clearfix"></div>

<!-- End Main Slider -->

<!-- Start Highlights -->
<div class="container highlights">
  	<h1 class="header-underline-orange"><span>Our Value</span> Proposition</h1>
  		<div class="clearfix"></div>

		  	<div class="col-md-3 col-xs-12 highlights-panel section-01">
		  		<img src="<?php print IMAGES; ?>/highlights-01.png" class="featured-img" />
    			<h3>Frequent business insight</h3>
		    	<p class="highlights-panel-paragraph">MFI Insight Analytics’s  Financial Analytics Platform<sup>SM</sup> (FAP<sup>SM</sup>) is an end-to-end Solution as a Service (SaaS) that gives you an up-to-date holistic view of your <label>...</label> business on a day-to-day basis. The processes have been automated in such a way that tasks such as Loan Portfolio Aging, Vintage Analysis and Cash Flow Management are quick and effortless. Additionally, it allows you to set and achieve frequent targets for the KPIs that matter to you, such as, PaR, Customer Churn Rate, Financial Ratios and Operating Cost Ratio.
		    	</p>
		    	<span class="highlights-panel-paragraph-more  glyphicon glyphicon-chevron-right set-4-info-more"></span>
		    </div>

		  	<div class="col-md-3 col-xs-12  highlights-panel section-02">
		  		<img src="<?php print IMAGES; ?>/highlights-02.png" class="featured-img" />
		    	<h3>Quick deployment</h3>
		    	<p class="highlights-panel-paragraph">FAP<sup>SM</sup> comes with predefined KPIs and insights based on industry-standard metrics, which can be tailored to your business. In total, we aim for an end-to-end <label>...</label>  implementation period of 3 – 4 months. The result of this quick deployment is drastic time-savings and a quicker return on investment.
		    	</p>
		    	<span class="highlights-panel-paragraph-more  glyphicon glyphicon-chevron-right set-4-info-more"></span>
		  	</div>

	  		<div class="col-md-3 col-xs-12 highlights-panel section-03">
	  			<img src="<?php print IMAGES; ?>/highlights-03.png" class="featured-img" />
	    		<h3>Flexibility</h3>
	    		<p class="highlights-panel-paragraph">FAP<sup>SM</sup> is both architecture agnostic and cloud ready. It runs on a highly-configurable core that plugs seamlessly into whichever enterprise architecture you have <label>...</label> installed. This saves you the additional cost and human resource requirement that comes with installing a new architecture. Alternatively, MFI Insight Analytics offers a cloud-hosted solution that takes the stress of managing the infrastructure off your hands so you can focus on running your business.
	    		</p>
	    		<span class="highlights-panel-paragraph-more  glyphicon glyphicon-chevron-right set-4-info-more"></span>
	  		</div>

  			<div class="col-md-3 col-xs-12 highlights-panel section-04">
  			    <img src="<?php print IMAGES; ?>/highlights-04.png" class="featured-img" />
    		   <h3>Extensibility</h3>
    			<p class="highlights-panel-paragraph">FAP<sup>SM</sup> is broken up into four loosely coupled modules, The Finance Analytics Module FAM<sup>SM</sup>, The Channel & Network Performance Module CNM<sup>SM</sup>, <label>...</label> The 360 Customer Centricity Module CCM<sup>SM</sup>and finally The Product Performance Module PPM<sup>SM</sup> .We can deploy the module that suits your business and budget. Should you require additional functionality in the future, activation of additional modules is fast and simple.
    		   </p>
				<span class="highlights-panel-paragraph-more  glyphicon glyphicon-chevron-right set-4-info-more"></span>
  			</div>
</div>
<!-- End Highlights -->

<!-- Start Tab Panel -->
<div class="tab-viewed-wrapper">
	  <div class="container tabbed-view-info">
	    <h1 class="header-underline-light-blue">Our differentiating factors</h1>
	    <p class="featured-text">We pride ourselves in supporting our clients, giving them the confidence to meet their immediate and long term objectives. <strong>No excuse, just results !</strong> </p>
	    <div role="tabpanel"> 
	      <!-- Nav tabs -->
	      <ul class="nav nav-tabs featured-tabs" role="tablist">
	        	<li role="presentation" class="active">
	        		<span class="glyphicon glyphicon-arrow-down"></span>
	        		<a href="#home" aria-controls="home" role="tab" data-toggle="tab">Fast Value Delivery</a>
	        	</li>
	        	<li role="presentation">
	        		<span class="glyphicon glyphicon-arrow-down"></span>
	        		<a href="#profile" aria-controls="profile" role="tab" data-toggle="tab">Results Committed</a>
	        	</li>
	        	<li role="presentation">
	        		<span class="glyphicon glyphicon-arrow-down"></span>
	        		<a href="#messages" aria-controls="messages" role="tab" data-toggle="tab">Our solution</a>
	        	</li>
	      </ul>
	      <!-- Tab panes -->
	      <div class="tab-content">
	        <div role="tabpanel" class="tab-pane active" id="home">
		         <h4>Fast Value Delivery</h4>
		          <p>We know the business of Microfinance Institutions very well, which enables us to achieve quick wins. We have extensive experience in solution implementation and integration, we are agile and cross-functional</p>
	        </div>
	        <div role="tabpanel" class="tab-pane fade" id="profile">
		       	<h4>Results Committed </h4>
		       	<p>We partner with you to meet your acceptance criteria and to conduct all the necessary training and coaching ensuring that both your operational and technical team understand the optimal use of the solution. MFI Insight Analytics also helps you define and set-up the perfect process on how to use the solution and integrates it into the day-to-day job of your team.</p>
	        </div>
	        <div role="tabpanel" class="tab-pane fade" id="messages">
	        	<h4>Our solution is embedded in your enterprise Architecture </h4>
	        	<p>We fully implement our solution in your system, connecting it directly to legacy systems and advising you on the architecture and tools that support our solution.
				</p>
				<p>MFI Insight Analytics team has 50 years of cumulative Business Intelligence Experience. We have been providing services to the leading retail banks such BNP-Paribas, AxA Bank, Dexia, Equity Bank, Opportunity International and SWIFT.
				</p>
	        </div>
	      </div>
	    </div>
	    <!-- tab panel container end --> 
	  </div>
</div>
<!-- End Tab Panle -->

<!-- Pateners Slider -->
<div class="clearfix"></div>
<div class="slider-panel-partners">
  <div class="container">
    <h1 class="header-underline-dull-blue"><span>we have achieved</span>
    <label> trust by the top global players.</label>
    </h1>
    
    <!-- slider start -->
    
    <div class="demo">
      <div class="clearfix"></div>
      <div class="item">
        <ul id="responsive" class="content-slider">
          <li> <img src="<?php print IMAGES; ?>/partner-logos/axa.jpg" /> </li>
          <li> <img src="<?php print IMAGES; ?>/partner-logos/bnp.jpg" /> </li>
          <li> <img src="<?php print IMAGES; ?>/partner-logos/bpost.jpg" /> </li>
          <li> <img src="<?php print IMAGES; ?>/partner-logos/dexia.jpg" /> </li>
          <li> <img src="<?php print IMAGES; ?>/partner-logos/dexia.jpg" /> </li>
          <li> <img src="<?php print IMAGES; ?>/partner-logos/electrabel.jpg" /> </li>
          <li> <img src="<?php print IMAGES; ?>/partner-logos/vodacom.jpg" /> </li>
          <li> <img src="<?php print IMAGES; ?>/partner-logos/vodafone.jpg" /> </li>
          <li> <img src="<?php print IMAGES; ?>/partner-logos/axa.jpg" /> </li>
          <li> <img src="<?php print IMAGES; ?>/partner-logos/bnp.jpg" /> </li>
          <li> <img src="<?php print IMAGES; ?>/partner-logos/bpost.jpg" /> </li>
          <li> <img src="<?php print IMAGES; ?>/partner-logos/dexia.jpg" /> </li>
          <li> <img src="<?php print IMAGES; ?>/partner-logos/dexia.jpg" /> </li>
          <li> <img src="<?php print IMAGES; ?>/partner-logos/electrabel.jpg" /> </li>
          <li> <img src="<?php print IMAGES; ?>/partner-logos/vodacom.jpg" /> </li>
          <li> <img src="<?php print IMAGES; ?>/partner-logos/vodafone.jpg" /> </li>
        </ul>
        <div class="lSAction">
        <a class="lSPrev"><span class="glyphicons glyphicon-chevron-left"></span></a>
        <a class="lSNext"><span class="glyphicons glyphicon-chevron-right"></span></a>
        </div>
      </div>
    </div>
    
    <!-- slider end--> 
  </div>
</div>